 ...
 *
 * IMPLEMENTATION:
 * - Tracks driver location every 4 seconds and sends to backend
 * - Polls trip live state every 4 seconds
 * - Displays trip route from start to current position
 * - Allows driver to end trip
 *
 * API Endpoints Used:
 * - PUT /api/rides/{rideId}/driver/location - Update driver location
 * - GET /api/rides/{rideId}/live - Poll trip state
...
export default function DriverTripInProgressScreen() {
  ...
  // Send driver location updates every 4 seconds
  useEffect(() => {
    if (!rideId) return;

    const sendTripLocation = async () => {
      try {
        ...
        console.log(
          `📍 Driver location sent: ${coords.latitude}, ${coords.longitude}`,
        );
      } catch (error) {
    ...
    // Start location updates every 4 seconds
    locationIntervalRef.current = setInterval(sendTripLocation, 4000);
    ...
    return () => {
      if (locationIntervalRef.current) {
        console.log("🛑 Stopping driver location updates");
  ...
  // ============= TRIP STATE POLLING =============
  // Poll trip live state every 4 seconds
  useEffect(() => {
    if (!rideId) return;
        ...
        if (response.success && response.data) {
          ...
          if (liveState.status === "COMPLETED") {
            ...
            toast.show({
              ...
              message: "Redirecting to summary...",
            });
  ...
  // Calculate route stats from pickup to drop-off location
  useEffect(() => {
    const calculateRoute = async () => {
      try {
        ...
        if (directions) {
          ...
          console.log(`✅ Trip stats calculated: ${directions.distance}km`);
        }
      } catch (error) {
  ...
  // ============= PROGRESS ROUTE (START → CURRENT) =============
  useEffect(() => {
    if (driverLocation) {
      setTripRoute([pickupCoords, driverLocation]);
    }
  }, [driverLocation, pickupCoords.latitude, pickupCoords.longitude]);
